    ...
    if "edit_cash" in st.session_state:
        ...
        with cols[0]:
            cash_init_balance = st.number_input(
                "初期残高（現金）",
                value=int(config_manger.get_cash_init_balance()),
            )
            ...
            if f"edit_bank_{i+1}" in st.session_state:
                ...
                with cols[1]:
                    ...
                    if st.button(
                        ...
                        config_manger.update_bank_accounts(
            ...
            if f"edit_credit_{i+1}" in st.session_state:
                ...
                with cols[1]:
                    ...
                    if st.button(
                        ...
                        config_manger.update_credit_cards(
    ...
    if edit_mode == "編集":
        ...
        if st.button(":material/save: 保存"):
            if st.session_state.get("income_df_changed", False):
                xor_list = list(
                    set(categories_df["income"])
                    ^ set(updated_categories_income)
                )
                if (len(categories_df["income"]) - len(xor_list) > 0) and any(
                    is_used_in_db("category", cat) for cat in xor_list
                ):
                    st.error(
                        ":material/Warning: \
            ...
            if st.session_state.get("expense_df_changed", False):
                xor_list = list(
                    set(categories_df["expense"])
                    ^ set(updated_categories_expense)
                )
                if (len(categories_df["expense"]) - len(xor_list) > 0) and any(
                    is_used_in_db("category", cat) for cat in xor_list
                ):
                    st.error(
                        ":material/Warning: \
        ...
        with row[2]:
            ...
            merge_target = st.selectbox(
                "統合先", category_list, key="merge_target_income"
            )
