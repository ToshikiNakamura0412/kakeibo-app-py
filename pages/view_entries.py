...
if view_type == "月別":
    ...
    selected_month = st.pills(
        "Month", month_options, key="selected_month", default=month_options[-1]
    )
    ...
    sum_income = (
        0 if len(agg_df_income) == 0 else agg_df_income["amount"].sum()
    )
    sum_expense = (
        0 if len(agg_df_expense) == 0 else agg_df_expense["amount"].sum()
    )
    ...
    avg_income = (
        0 if len(agg_df_income) == 0 else agg_df_income["amount"].mean()
    )
    # - expense
    sum_expense = (
        0 if len(agg_df_expense) == 0 else agg_df_expense["amount"].sum()
    ...
    avg_expense = (
        0 if len(agg_df_expense) == 0 else agg_df_expense["amount"].mean()
    )
