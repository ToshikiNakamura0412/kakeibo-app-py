...
from kakeibo.common import config_manager, config_models, ui_components
...
if mode == "表示モード":
    ...
    with row[0]:
        ...
        start_date = st.date_input(
            "開始日",
            value=pd.to_datetime(f"{df['year'].min()}-{df['month'].min()}-01"),
        )
    with row[1]:
    ...
    df = df[
        (pd.to_datetime(df["日付"]) >= pd.to_datetime(start_date))
        & (pd.to_datetime(df["日付"]) <= pd.to_datetime(end_date))
    ]
