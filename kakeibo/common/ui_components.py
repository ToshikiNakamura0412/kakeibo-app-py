...
from kakeibo.common import config_manager, config_models
...
def render_sidebar():
    with st.sidebar:
        ...
        st.page_link(
            "pages/input_form.py", label=":material/Input: 入力フォーム"
        )
        st.page_link(
            "pages/view_entries.py", label=":material/Bar_Chart: 照会画面"
        )
        st.page_link("pages/edit_entries.py", label=":material/Edit: 編集画面")
        st.page_link(
            "pages/import_form.py", label=":material/Place_Item: インポート"
        )
...
def render_input_form(
    selected_entry=pd.Series(), selected_id=None, update_button_label="保存"
) -> None:
    ...
    with row[0]:
        if "date" in selected_entry.index:
            entry.date = str(
                st.date_input("日付：", pd.to_datetime(selected_entry["date"]))
            )
        else:
    ...
    if "transaction_type" in selected_entry.index:
        entry.transaction_type = str(
            st.segmented_control(
                "",
                ["収入", "支出"],
                default=selected_entry["transaction_type"],
            )
        )
    else:
        entry.transaction_type = str(
            st.segmented_control("", ["収入", "支出"], default="支出")
        )
    ...
    for i in range(1, 11):
        if config_manger.is_in_config(f"credit_card_{i}"):
            ...
            payment_method_options.append(
                f"{credit_card_config.name}（クレジット）"
            )
        else:
            break
    ...
    try:
        entry.payment_method = str(
            st.segmented_control(
                "",
                payment_method_options,
                default=selected_entry["payment_method"],
            )
        )
    except KeyError:
        entry.payment_method = str(
            st.segmented_control(
                "", payment_method_options, default=payment_method_options[0]
            )
        )

    # category
    category_options = (
        config_manger.categories_df["income"]
        if entry.transaction_type == "収入"
        else config_manger.categories_df["expense"]
    )
    category_options = list(category_options)
    category_options.extend(
        df[(df["transaction_type"] == entry.transaction_type)][
            "category"
        ].tolist()
    )
    ...
    try:
        entry.category = str(
            st.pills("", category_options, default=selected_entry["category"])
        )
    except KeyError:
        entry.category = str(
            st.pills("", category_options, default=category_options[0])
        )
    ...
    with row[1]:
        if "amount" in selected_entry.index:
            entry.amount = str(
                st.number_input(
                    "金額：", min_value=0, value=int(selected_entry["amount"])
                )
            )
        else:
...
def render_current_balance() -> None:
    ...
    if len(bank_list) > 0:
        ...
        for bank in bank_list:
            ...
            st.write(
                f"{bank_name}：{bank_balance:,} 円\
                    （未請求額：{unbilled_amount:,} 円）"
            )
