...
class ConfigManager:
    ...
    def get_bank_account(
        self, index: int = 0
    ) -> config_models.BankAccountConfig:
    ...
    def renumber_configs(self):
        ...
        existing_bank_accounts = [
            key for key in bank_account_keys if self.is_in_config(key)
        ]
        for i, key in enumerate(existing_bank_accounts):
            if key != f"bank_account_{i + 1}":
                self.config_df.loc[f"bank_account_{i + 1}"] = (
                    self.config_df.loc[key]
                )
        ...
        existing_credit_cards = [
            key for key in credit_card_keys if self.is_in_config(key)
        ]
        for j, key in enumerate(existing_credit_cards):
            if key != f"credit_card_{j + 1}":
                self.config_df.loc[f"credit_card_{j + 1}"] = (
                    self.config_df.loc[key]
                )
                self.config_df = self.config_df.drop(index=key)
