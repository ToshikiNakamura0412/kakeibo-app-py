		...
		for i in range(1, 11):
			if config_manger.is_in_config(f'credit_card_{i}'):
				...
				if bank_name ==bank_account['name']:
					bank_account['credit_card'].append(credit_card_config.to_dict())
			else:
				break
...
class Aggregation:
	...
	_bank_and_credit_card_list = create_bank_and_credit_card_list(_config_manger)
	...
	def sum_cash_income(self, df: pd.DataFrame) -> int:
		try:
			cash_income = df[(df['transaction_type'] == '収入') & (df['payment_method'] == '現金')]
	...
	def sum_cash_expenses(self, df: pd.DataFrame) -> int:
		try:
			cash_expenses = df[(df['transaction_type'] == '支出') & (df['payment_method'] == '現金')]
	...
	def sum_bank_income(self, df: pd.DataFrame, bank_name: str) -> int:
		try:
			bank_income = df[(df['transaction_type'] == '収入') & (df['payment_method'] == f'{bank_name}（銀行）')]
	...
	def sum_bank_expenses(self, df: pd.DataFrame, bank_name: str) -> int:
		try:
			bank_expenses = df[(df['transaction_type'] == '支出') & (df['payment_method'] == f'{bank_name}（銀行）')]
	...
	def sum_credit_card_expenses(self, df: pd.DataFrame, bank_name: str) -> int:
		credit_cards = []
		...
		for credit_card in credit_cards:
			...
			try:
				credit_card_expenses = df[(df['transaction_type'] == '支出') & (df['payment_method'] == f'{credit_card_name}（クレジット）')]

				today = datetime.today()
				if not credit_card_expenses.empty:
					if self.make_valid_date(today.year, today.month, int(credit_card['payment_day'])) <= today:
						date_th = self.make_valid_date(today.year, today.month - 1, int(credit_card['closing_day']))
					else:
						date_th = self.make_valid_date(today.year, today.month - 2, int(credit_card['closing_day']))
					credit_card_expenses = credit_card_expenses[pd.to_datetime(credit_card_expenses['date']) <= date_th]
